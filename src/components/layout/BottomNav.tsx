...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 px-2 pt-2 pb-[max(0.75rem,env(safe-area-inset-bottom))] pointer-events-none">
      <nav className="max-w-lg mx-auto bg-slate-950/90 dark:bg-slate-950/95 backdrop-blur-3xl border border-slate-800/90 dark:border-slate-800/90 rounded-3xl shadow-2xl p-1.5 flex items-center justify-around pointer-events-auto">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          const isActive =
            activeTab === tab.id ||
          ...
          return (
            <button
              ...
              className={`flex-1 py-2 px-0.5 sm:px-1 flex flex-col items-center justify-center gap-1 rounded-2xl transition-all interactive-tap touch-target ${
                isActive
                  ? 'bg-gradient-to-b from-amber-500/25 to-gold-500/10 text-gold-300 font-bold border border-gold-500/40'
                  : 'text-slate-400 hover:text-slate-200 dark:hover:text-slate-200 hover:bg-slate-900/40'
              }`}
            >
              <Icon className={`w-4 h-4 sm:w-5 sm:h-5 transition-transform ${isActive ? 'scale-110 text-gold-400' : ''}`} />
