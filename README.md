...
# 🪙 ترازینو (Tarazino)
### دستیار هوشمند مدیریت پس‌انداز و سبد سرمایه‌گذاری (ترکیب طلای بورسی و رمزارزها)
...
| کاربرد | بخش |
| ---: | ---: |
| نمایش ارزش کل دارایی‌ها، نمودار ترکیب سبد، کارت ورودی مبلغ پس‌انداز و دستور خرید پیشنهادی. | **📊 داشبورد** |
| مشاهده قیمت‌های لحظه‌ای صندوق‌های طلای بورس (عیار، طلا، کهربا، زر و...) و مدیریت واحدهای خریداری‌شده. | **🪙 بورس و طلا** |
| اتصال به صرافی نوبیتکس، مشاهده مانده ریالی، موجودی کوین‌ها و قیمت‌های لحظه‌ای رمزارزها. | **⚡ ارز دیجیتال** |
| فهرست جامع تمامی دارایی‌ها به همراه ثبت تاریخچه خریدها. | **💼 دارایی‌ها** |
| تغییر درصد پس‌انداز، نسبت طلا به کریپتو، سهم هر ارز و دریافت فایل پشتیبان. | **⚙️ تنظیمات** |
...
### مراحل گام‌به‌گام دریافت کلید در سایت نوبیتکس:

1. به سایت [nobitex.ir](https://nobitex.ir) بروید و از منوی بالای صفحه بخش **امکانات**، گزینه **مستندات API** را انتخاب کنید و از آنجا روی دکمه **ایجاد کلید API** کلیک کنید (یا مستقیماً به آدرس **[nobitex.ir/panel/profile/api-services](https://nobitex.ir/panel/profile/api-services/)** بروید).
2. روی دکمه **«کلید جدید +»** کلیک کنید.
3. در پنجره بازشده:
   - یک نام برای آن انتخاب کنید (مثلاً: `برنامه مدیریت پس‌انداز`).
   - در بخش **مجوزها** به چیزی دست نزنید (تنظیمات پیش‌فرض فقط خواندن کافی است).
   - مطمئن شوید بخش **مدت اعتبار** روی گزینه **دائمی** باشد.
   - در بخش **محدودیت دسترسی**:
     - برای اغلب کاربران روی **بدون محدودیت** قرار دهید.
     - در صورتی که امنیت کامل می‌خواهید، روی **فقط IPهای مجاز** بگذارید و آدرس IP خود را وارد کنید.
4. روی دکمه **«ایجاد کلید»** کلیک کنید و تایید دومرحله‌ای/پیامک را انجام دهید.
5. نوبیتکس دو کلید به شما نمایش می‌دهد:
   - **کلید عمومی (API Key / Public Key)**
   - **کلید خصوصی (Secret Key / Private Key)**
6. وارد اپلیکیشن ترازینو شوید، در تب **«ارز دیجیتال»** دکمه **«اتصال به نوبیتکس»** را لمس کنید و کلیدها را در کادرهای مربوطه قرار داده و دکمه **«ذخیره و همگام‌سازی»** را بزنید.
...
2. آخرین نسخه فایل نصبی با پسوند **`.apk`** (مانند `Tarazino-v1.0.X.apk`) را دانلود کرده و مستقیماً روی گوشی خود لمس و نصب نمایید.
...
1. از بخش **[Releases](../../releases)** فایل با پسوند **`.ipa`** (مانند `Tarazino-v1.0.X.ipa`) را دانلود کنید.
